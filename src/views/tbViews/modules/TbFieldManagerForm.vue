    ...
    <j-form-container :disabled="formDisabled">
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          ...
          <a-col :span="24">
            <a-form-model-item label="字段是否是字典" :labelCol="labelCol" :wrapperCol="wrapperCol"
                               prop="fieldIsDictFlag">
              <j-dict-select-tag type="list" v-model="model.fieldIsDictFlag" dictCode="is_dict"
                                 placeholder="请选择字段是否是字典"/>
            </a-form-model-item>
          </a-col>
          <a-col :span="24">
...
<!--              <j-dict-select-tag type="list" v-model="model.fieldId" dictCode="is_dict"-->
<!--                                 placeholder="请选择字段是否是字典"/>-->
              <a-select
                  ...
                  @change="handleChange"
              >
                <a-select-option v-for="d in dataValues" :key="d.value">{{d.value}}:{{ d.text }}</a-select-option>
              </a-select>
...
import {fetch} from "@api/getDictForSearch";
  ...
  data() {
    return {
      ...
      url: {
        ...
        getDictByName: "/sys/api/queryEnableDictItemsByCode"
      }
    }
  },
  computed: {
